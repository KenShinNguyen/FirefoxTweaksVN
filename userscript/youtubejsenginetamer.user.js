// ==UserScript==
// @name        YouTube JS Engine Tamer (FirefoxTweaksVN fork)
// @name:ja     YouTube JS Engine Tamer (FirefoxTweaksVN fork)
// @name:zh-TW  YouTube JS Engine Tamer (FirefoxTweaksVN fork)
// @name:zh-CN  YouTube JS Engine Tamer (FirefoxTweaksVN fork)
// @namespace   FirefoxTweaksVN
// @version     0.42.25-vn1
// @match       https://www.youtube.com/*
// @match       https://www.youtube-nocookie.com/embed/*
// @match       https://studio.youtube.com/live_chat*
// @license     MIT
// @author      CY Fung (fork patched in FirefoxTweaksVN)
...
// Fork of https://greasyfork.org/scripts/473972 (upstream v0.42.25, MIT, by CY Fung).
// No @updateURL / @downloadURL on purpose: the userscript manager must not
// silently replace this file with upstream and drop the patches below.
//
// Changes against upstream, all marked with `[VN fork]` in the source:
//   1. FIX_error_many_stack / SCRIPTLET_REMOVE_PRUNE_propNeedles turned off -
//      upstream reaches into uBlock Origin's json-prune scriptlet internals and
//      neutralises them; that also stops upstream from replacing JSON.parse.
//   2. FULLY_REMOVE_ALL_EVENT_LISTENERS / MEMORY_RELEASE_MAP_SET_REMOVE_NODE
//      turned off - their only consumer sits behind MEMORY_RELEASE_NF00, which
//      upstream ships disabled, so they only taxed every addEventListener /
//      Set.add / Map.set on the page.
//   3. OVERRIDE_isConnected added (off) - the upstream getter replaced the
//      native Node.prototype.isConnected fast path with a flag that is never set.
//   4. Node.prototype.removeChild: the internalByPass latch is released in a
//      `finally`, and childNodes.indexOf is used only when it exists.
//   5. Node.prototype.appendChild: only a fragment with no child nodes at all is
//      skipped; a comment-only fragment is appended like the DOM requires.
//   6. requestIdleCallback: the callback runs directly once the page has loaded,
//      so the IdleDeadline handed to it is still valid.
//   7. copyPreviousContiuationToIgnored374: `.size` instead of `.length`, and the
//      Set is iterated by value instead of being destructured as a pair. This is the
//      one change that revives logic that never ran upstream, so it sits behind
...
(() => {
  ...
  */
  ...
  if (OVERRIDE_isConnected && Node.isConnectedOverrided === undefined) { // [VN fork] gated; see OVERRIDE_isConnected
  ...
  if (WEAK_CE_ROOT) {
    Object.defineProperty(Object.prototype, '__CE_shadowRoot', {
      configurable: true, // [VN fork] upstream leaves this non-configurable, so the patch can never be undone
      enumerable: false,
      get() {
  ...
  FIX_removeChild && (() => {
    if (typeof Node.prototype.removeChild === 'function' && typeof Node.prototype.removeChild062 !== 'function') {
      ...
      // [VN fork] childNodes is a plain array only once ShadyDOM has patched it;
      // calling indexOf on a real NodeList used to throw a TypeError out of removeChild.
      const shadyIndexOf = (parent, node) => {
        const childNodes = parent.childNodes;
        if (!childNodes || typeof childNodes.indexOf !== 'function' || !(childNodes.length >= 1)) return -1;
        return childNodes.indexOf(node);
      };
      // [VN fork] the latch is released in a `finally`; upstream skipped it on a throw,
      // which left internalByPass stuck true and every later failed removeChild a silent no-op.
      const detachViaFragment = (parent, node, idx) => {
        internalByPass = true;
        try {
          node.parentNode !== fragD && fragD.appendChild4201(node);
          parent.childNodes[idx] === node && typeof parent.childNodes.splice === 'function' && parent.childNodes.splice(idx, 1);
          fragD.removeChild4201(node);
        } finally {
      ...
      Node.prototype.removeChild = function (child) {
        ...
        if (this instanceof Node && child instanceof Node && this.nodeType === 11 && child.parentNode !== this && this.contains(child)) { // eg. child = DOM-IF
          let idx = shadyIndexOf(this, child);
          if (idx >= 0) {
            return detachViaFragment(this, child, idx);
          }
        }
...
// }
  ...
  if (FIX_fix_requestIdleCallback_timing && !window.requestIdleCallback471 && typeof window.requestIdleCallback === 'function') {
    const requestIdleCallback471 = window.requestIdleCallback471 = window.requestIdleCallback;
    // [VN fork] once the page has loaded the callback runs straight away, so the IdleDeadline it
    // receives is still live. Upstream always awaited pLoad first, which handed every callback an
    // already-expired deadline whose timeRemaining() reads ~0. `this` is captured instead of
    // `(this || window)`, which threw whenever requestIdleCallback was called on another object.
    let pLoadDone = false;
    pLoad.then(() => { pLoadDone = true; });
    window.requestIdleCallback = function (f, ...args) {
      return requestIdleCallback471.call(this || window, function (...cbArgs) {
        if (pLoadDone) return f.call(this, ...cbArgs);
        return pLoad.then(() => f.call(this, ...cbArgs));
      }, ...args);
    }
  }
    ...
    */
      ...
      if (f) Node.prototype.appendChild = function (a) {
        if (this instanceof Element) { // exclude DocumentFragment
          try {
            ...
            // [VN fork] upstream skipped any fragment without an element child unless it held a
            // text node, so a comment-only fragment (Polymer template placeholders) was silently
            // dropped. Skipping a fragment with no child nodes at all is what the DOM does anyway.
            if (checkFragmentA && a.firstElementChild === null && a.firstChild === null) {
              return a;
            }
          } catch (e) {
            console.log(e);
    ...
    !isUrlInEmbed && Promise.resolve().then(() => {
      ...
      function copyPreviousContiuationToIgnored374(toClearRecorded) {
        ...
        // [VN fork] mfvContinuationRecorded is a Set of continuation strings: upstream read
        // `.length` (always undefined, so the whole function was dead) and destructured each
        // string as a pair, which would have recorded its first two characters instead.
        if (FIX_CONTINUATION_IGNORE_COPY && mfvContinuationRecorded.size > 0) {
